...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!userDoc.exists) {
            return NextResponse.json({ ok: false, message: "User tidak valid" }, { status: 400 });
        }

        // Fetch Tier details
        const tierDocRef = adminDb.collection("subscription_tiers").doc(parsed.tier_id);
        const tierDoc = await tierDocRef.get();

        if (!tierDoc.exists) {
            return NextResponse.json({ ok: false, message: "Tier langganan tidak ditemukan" }, { status: 404 });
        }

        const tierData = tierDoc.data();
        if (!tierData?.is_active) {
            return NextResponse.json({ ok: false, message: "Tier langganan sudah tidak aktif" }, { status: 400 });
        }
