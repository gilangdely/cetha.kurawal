...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (
      rawMessage.toLowerCase().includes("pdf kosong") ||
      rawMessage.toLowerCase().includes("tidak dapat dibaca")
    ) {
      return NextResponse.json(
        {
          success: false,
          message:
            "PDF berhasil diunggah, tetapi teks CV tidak terbaca. Pastikan file bukan hasil scan gambar, tidak terenkripsi/password, dan bisa di-select/copy text dari PDF.",
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Kuota API Server terlampaui. Silakan coba lagi beberapa saat lagi.",
        },
        { status: 429 },
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        message:
          error?.message ||
          "Terjadi kesalahan saat memproses file (Internal Server Error).",
      },
      { status: 500 },
    );
  }
